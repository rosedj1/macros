 ...
 //	massPoint.push_back(130);
	...
	for(int y = 0; y < year.size(); y++){
		...
		for(int PM = 0; PM < ProdMode.size(); PM++){
		    	...
		    	TString nome_file = "../../../HZZ4l_Run2_d0/CMSSW_10_2_15/src/DUMMYFILENAME_2016";
                    //Form("root://cmsio5.rc.ufl.edu//store/user/t2/users/ferrico/Full_RunII/%s/%s_M%i_%s", ProdMode[PM].Data(), ProdMode_File[PM].Data(), massPoint.at(mP), year[y].Data());
...
//   newtree->Branch("etaj2",&etaj2,"etaj2/D");
			    	...
			    	TString nome_file_output = "Skimmed_2016";
                        //Form("/raid/raid8/ferrico/Useful_Code_HZZ/CMSSW_10_2_15/src/Full_RunII/%s/%s_M%i_%s", ProdMode[PM].Data(), ProdMode_File[PM].Data(), massPoint.at(mP), year[y].Data());
		...
		for(int Bg = 20; Bg < Background.size(); Bg++){
            if(y != 0) continue;
