...
void Draw(TH1F *h1, TH1F *h2, TH1F *h3, TString nome_canvas, TString save, TLegend *legend, TString fs);
...
void YearComparison_Data(TString fs, TString run){
	...
	for(int mu = 0; mu < 1; mu++){
		...
		for(int year = 0; year < 3; year++){
			...
			if(year == 0){
			    if(fs=="2mu") infile_name = "/raid/raid8/ferrico/Useful_Code_HZZ/CMSSW_10_2_15/src/Full_RunII/Data/2016/Data_m" + fs + run + ".root";
			    else infile_name = "/raid/raid8/ferrico/Useful_Code_HZZ/CMSSW_10_2_15/src/Full_RunII/Data/2016/Data_m" + fs + run + ".root";
	        }		
	        else if(year == 1){                                                                                                                      
			    if(fs=="2mu") infile_name = "/raid/raid8/ferrico/Useful_Code_HZZ/CMSSW_10_2_15/src/Full_RunII/Data/2017/Data_m" + fs + run + ".root";
        	    else infile_name = "/raid/raid8/ferrico/Useful_Code_HZZ/CMSSW_10_2_15/src/Full_RunII/Data/2017/Data_m" + fs + run + ".root";
			}
			else{
			    if(fs=="2mu") infile_name = "/raid/raid8/ferrico/Useful_Code_HZZ/CMSSW_10_2_15/src/Full_RunII/Data/2018/Data_m" + fs + run + ".root";
            	else infile_name = "/raid/raid8/ferrico/Useful_Code_HZZ/CMSSW_10_2_15/src/Full_RunII/Data/2018/Data_m" + fs + run + ".root";
	        }
			...
			std::cout<<nentries_MU<<"\t"<<infile_name<<std::endl;	
...
// 	 		for(int entry = 0; entry < 1000; entry++){
				...
				if(entry % 1000000 == 0){         
					if(fs=="2mu") std::cout<<entry<<" --- Dentro il TREE "<<year<<" --- MUON "<<year<<std::endl; 
...
// 	return;
		...
		if(mu == 0){
			...
			legend_year->AddEntry(histograms[0][1][0], "2016");
			legend_year->AddEntry(histograms[0][1][1], "2017");
			legend_year->AddEntry(histograms[0][1][2], "2018");
		}

	
		directory = "./PlotYearComparison_Data";
...
// 	Draw(h_blank, h_blank, "h_blank", save_nome + ".pdf]", legend_reco_gen);
		...
		Draw(h_blank, h_blank, h_blank, "h_blank", directory + save_nome + ".pdf[", legend_reco_gen, fs);
...
// 				if(var == 0 || var == 4 || var == 5) continue; 
				...
				if(i == 1) continue;
				Draw(histograms[i][var][0], histograms[i][var][1], histograms[i][var][2], nome_canvas, directory + save_nome + ".pdf", legend_year, fs);
				Draw(histograms[i][var][0], histograms[i][var][1], histograms[i][var][2], nome_canvas, directory + nome_canvas + ".png", legend_year, fs);
			}
		}
